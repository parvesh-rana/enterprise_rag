...
async def fetch_all(tickers: list[str], dest_root: Path) -> list[tuple[FilingRef, Path]]:
    """Concurrently fetch the latest 10-K for each ticker. Sequential within
    EDGAR rate limits via a small semaphore."""
    sem = asyncio.Semaphore(3)

    async with httpx.AsyncClient() as client:

        async def one(t: str) -> tuple[FilingRef, Path]:
            async with sem:
                ref = await latest_10k(client, t)
